...
const typeNames = Object.keys(types).reduce(
    function(r, k) {
        r[types[k]] = k;
        return r;
    },
...
export class Node {
    ...
    addNull() {
        return this.add(types.NULL, Buffer.alloc(0));
    }
    addInt(val: number) {
        if (val < 0 || val >= 128) throw new Error('NIY: sorry only small positive integers supported');
        return this.add(types.INTEGER, Buffer.from([val]));
    }
    ...
    getMillis() {
        if (this.type === types.TIMESTAMP) {
            ...
            if (r) {
                const t =
                    (r[1] ? r[1] : '20') + r[2] + '-' + r[3] + '-' + r[4] + ' ' + r[5] + ':' + r[6] + ':' + r[7] + r[9];
                return Date.parse(t);
            }
    ...
    toString(pfx?: string): string {
        ...
        if (this.type & COMPOSITE) return head(name) + this.children!.map(toStr).join('');
            ...
            return (
                head(name) +
                b.toString(t === types.STRING || t === types.T61STRING || t === types.UTF8STRING ? 'utf8' : 'hex') +
                (t === types.OID ? ': ' + oids.toString(b) : '')
            );
    }
}
